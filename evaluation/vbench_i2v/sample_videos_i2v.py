...
def load_prompts(path: str) -> list[dict]:
    """Load prompts from either vbench2_i2v_full_info.json or prompts_i2v.json."""
    with open(path) as f:
        data = json.load(f)
    prompts = []
    for entry in data:
        prompts.append(
            {
                ...
                "dimensions": entry.get("dimensions", entry.get("dimension", [])),
...
def expand_steps_per_chunk(steps_per_chunk: list[int] | None, num_chunks: int, default_steps: int) -> list[int]:
    if not steps_per_chunk:
        return [default_steps] * num_chunks
    step_counts = [int(step) for step in steps_per_chunk[:num_chunks]]
    if len(step_counts) < num_chunks:
        step_counts.extend([step_counts[-1]] * (num_chunks - len(step_counts)))
...
def load_and_encode_image(image_path: str, tokenizer: Wan2pt1VAEInterface, w: int, h: int) -> torch.Tensor:
    """Load an image, resize to (w, h), VAE-encode to a single latent frame.

    Returns:
        image_latent: [1, C, 1, H_lat, W_lat] in bfloat16
    """
    ...
    img_tensor = img_tensor.to(**TENSOR_KWARGS)
    latent = tokenizer.encode(img_tensor)  # [1, C_lat, 1, H_lat, W_lat]
    return latent
...
def sample_causal_i2v(
    ...
    """Causal I2V sampling: prefill chunk 0 from image latent, denoise chunks 1..N.

    Args:
        image_latent: [B, C, 1, H, W] -- VAE-encoded first frame.
        init_noise:   [B, C, T-1, H, W] -- noise for remaining frames (excludes frame 0).
    """
    ...
    assert T_remaining % chunk_t == 0, f"T_remaining={T_remaining} not divisible by chunk_t={chunk_t}"
    num_blocks = 1 + T_remaining // chunk_t
    ...
    if not ode:
        ...
        for _ in range(max_steps):
            n = torch.randn(noise_full.shape, dtype=torch.float32, device=noise_full.device, generator=generator)
            step_noises.append(broadcast(n, cp_group) if cp_group is not None else n)
    else:
        step_counts = [len(t_steps) - 1] * (num_blocks - 1)
        step_noises = None

    # ---- Chunk 0: prefill image latent (no denoising) ----
    ...
    if context_from_last_step and context_from_last_step_start_chunk == 0:
        ...
        if cp_group is not None:
            image_noise = broadcast(image_noise, cp_group)
    ...
    inf_append_cond = CausalInferenceState(mode=KVCacheMode.APPEND, kv_caches=kv_cond, pattern=bp, block_cursor=0)
    net(
        x_B_C_T_H_W=image_context.to(**TENSOR_KWARGS),
        timesteps_B_T=prefill_t_B_1,
        **condition,
        inference_state=inf_append_cond,
        attn_meta=attn_meta_0,
    )
    if use_cfg:
        inf_append_uncond = CausalInferenceState(mode=KVCacheMode.APPEND, kv_caches=kv_uncond, pattern=bp, block_cursor=0)
        net(
            x_B_C_T_H_W=image_context.to(**TENSOR_KWARGS),
            timesteps_B_T=prefill_t_B_1,
            **uncondition,
    ...
    for i in range(1, num_blocks):
        ...
        if not use_last_step_context:
            ...
            inf_append = CausalInferenceState(mode=KVCacheMode.APPEND, kv_caches=kv_cond, pattern=bp, block_cursor=i)
            net(x_B_C_T_H_W=x.to(**TENSOR_KWARGS), timesteps_B_T=zero_t, **condition, inference_state=inf_append, attn_meta=attn_meta)
            if use_cfg:
                inf_append_u = CausalInferenceState(mode=KVCacheMode.APPEND, kv_caches=kv_uncond, pattern=bp, block_cursor=i)
                net(x_B_C_T_H_W=x.to(**TENSOR_KWARGS), timesteps_B_T=zero_t, **uncondition, inference_state=inf_append_u, attn_meta=attn_meta)
...
def parse_args():
    p = argparse.ArgumentParser(description="Distributed VBench-I2V video sampling (causal)", formatter_class=argparse.RawTextHelpFormatter)

    p.add_argument("--distilled", action="store_true", help="Few-step distilled sampling (default: multi-step ODE)")
    p.add_argument("--prompt_json", type=str, required=True)
    p.add_argument("--image_dir", type=str, required=True, help="Directory with cropped images (e.g. data/crop/16-9)")
    ...
    p.add_argument("--mid_t", type=str, nargs="*", default=None)
    p.add_argument("--steps_per_chunk", type=int, nargs="*", default=None, help="Distilled steps per generated chunk; repeats last value, e.g. 4 2")
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="First causal chunk index to use --context_from_last_step; I2V image prefill is chunk 0",
    )
...
def main():
    ...
    if args.output_dir is None:
        mode_tag = "distilled" if is_distilled else "ode"
        ckpt_stem = Path(args.dit_path).stem
        args.output_dir = str(Path(args.prompt_json).parent / f"videos_i2v_{mode_tag}_{args.num_steps}steps_{ckpt_stem}_seed{args.seed}")

    log.info(f"[Rank {rank}/{world_size}] I2V causal, distilled={is_distilled}, cp_size={args.cp_size}, dp_rank={dp_rank}/{dp_size}")

    # ---- Load & partition prompts by data-parallel rank ----
    all_prompts = load_prompts(args.prompt_json)
    my_prompts = all_prompts[dp_rank::dp_size]
    ...
    for p in tqdm(my_prompts, desc=f"[R{rank}] Text enc", disable=(rank != 0)):
        emb = get_umt5_embedding(checkpoint_path=args.text_encoder_path, prompts=p["prompt"])
    ...
    T_latent = tokenizer.get_latent_num_frames(args.num_frames)
    T_remaining = T_latent - 1
    noise_shape = [
        tokenizer.latent_ch,
        T_remaining,
    ...
    if is_distilled:
        ...
        num_generated_chunks = T_remaining // args.chunk_t
        step_counts = expand_steps_per_chunk(args.steps_per_chunk, num_generated_chunks, args.num_steps)
    ...
    if step_counts is not None and args.steps_per_chunk:
        log.info(f"Per-generated-chunk distilled steps: {step_counts}")
    log.info(f"Latent: T_total={T_latent} (1 image + {T_remaining} generated), chunk_t={args.chunk_t}")
    ...
    for p_idx, prompt_info in enumerate(my_prompts):
        ...
        image_name = prompt_info["image_name"]
        fname_base = sanitize_filename(prompt)

        image_path = os.path.join(args.image_dir, image_name)
        if not os.path.exists(image_path):
            log.warning(f"[Rank {rank}] Image not found: {image_path}, skipping")
            pbar.update(1)
            continue
        ...
        if not all_exist:
            image_latent = load_and_encode_image(image_path, tokenizer, w, h)

            # Sync image latent across CP group
            if cp_group is not None:
                from rcm.utils.context_parallel import broadcast

                image_latent = broadcast(image_latent, cp_group)
            ...
            for i in range(args.num_samples):
                ...
                init_noise = torch.randn(1, *noise_shape, dtype=torch.float32, device=device, generator=generator)
        ...
        if is_cp_leader:
            entry = {
                ...
                "image_type": prompt_info.get("image_type", ""),
            }
